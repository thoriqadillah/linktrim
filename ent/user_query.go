...
func (uq *UserQuery) loadLinks(ctx context.Context, query *LinkQuery, nodes []*User, init func(*User), assign func(*User, *Link)) error {
	...
	for _, n := range neighbors {
		fk := n.user_links
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_links" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_links" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
